...
<section class="container-fluid">
    ...
    <div class="row text-center" id="weg" style="direction: rtl">
        ...
        @if(\Illuminate\Support\Facades\Auth::check() and  \Illuminate\Support\Facades\Auth::user()->role == 'ADM')
            <form id="formsnow" method="post" action="{{route('saveeditcontentuseradmin')}}" enctype="multipart/form-data">
                @csrf
       ...
       $get = \Illuminate\Support\Facades\DB::table('links')->where('id',$link)->get()->toArray();
        ...
        $enc = \Illuminate\Support\Facades\Crypt::encrypt($enhash);
                ...
                <br><br>
                <input type="text" value="{{$keyword}}" name="sponser" placeholder="اسپانسر" maxlength="250">
                <br><br>
...
$id = $link->id;
            ...
            <form id="formsnow" method="post" action="{{route('saveeditcontentuser')}}" enctype="multipart/form-data">
                @csrf
                @foreach($content as $text)
                    @php
                        $id = $text->id;
